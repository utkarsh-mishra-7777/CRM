...
def activate_stored_messages_to_user(request: WSGIRequest, profile: UserProfile) -> None:
    if not profile or not hasattr(profile, 'messages'):
        return
...
def check_user_language(profile: UserProfile) -> None:
    def check_user_language(profile: UserProfile) -> None:
        if not profile:
            return  # Exit early if profile is None

    cur_language = get_language()

    if hasattr(profile, 'language_code') and cur_language != profile.language_code:
