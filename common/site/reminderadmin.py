...
class ReminderAdmin(admin.ModelAdmin):
    ...
    def to_object(self, obj):
        ...
        if not instance:
            return mark_safe('<span style="color: red;">[Missing Object]</span>')

        name = instance._meta.verbose_name
        url = reverse(
            f"site:{instance._meta.app_label}_{instance._meta.model_name}_change",
            args=(instance.id,)
        )
        return mark_safe(
            f'<a href="{url}" target="_blank">{name}: {instance}</a>'
        )
